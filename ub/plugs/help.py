 ...
 async def help(event):
    ...
    if args:
        if args in CMD_HELP:
            ...
            await event.edit("`Unknown module type !help to see all plugs`")
    else:
...
async def help(event):
    ...
    if args:
        if args in CMD_HELP:
            ...
            await event.reply("`Unknown module type !help to see all plugs`")
    else:
...
CMD_HELP.update({
    "filter":
    "!checkfilter\
    \nUsage: Lists all active ub filters in a chat.\
...
CMD_HELP.update({
    "stickers":
    "!kang\
\nUsage: Reply !kang to a sticker or an image to kang it to your ub pack.\
\n\n!kang [emoji('s)]\
