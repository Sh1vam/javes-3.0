...
async def __(event):
    ...
    os.remove(download)
@bot.on(admin_cmd(pattern="iresize ?(.*)", outgoing=True))
# DONOT KANG by Sh1vam
async def __(event):
...
@bot.on(admin_cmd(pattern="sresize ?(.*)", outgoing=True))
# DONOT KANG by Sh1vam
async def __(event):
    ...
    img=shvm.resize((int(shi),int(vam)))
    #await event.edit(f"Dimensions Of Image are {shi} by {vam}")
    img.save("sh1vam.webp", format="PNG", optimize=True)
    await event.client.send_file(event.chat_id, "sh1vam.webp", force_document=True, reply_to=event.reply_to_msg_id)
    await event.client.send_file(event.chat_id, "sh1vam.webp", force_document=False, reply_to=event.reply_to_msg_id)
    os.remove(download)
    os.remove("sh1vam.webp")
