...
async def handler(rkG): 
   if rkG.user_joined or rkG.user_added:      
       try:       	
         from ub.plugs.sqls.gmute_sql import is_gmuted
...
async def muter(rkG):
    try:        
        from ub.plugs.sqls.gmute_sql import is_gmuted
        from ub.plugs.sqls.globelmute_sql import is_globelmuted
    except:
        return    
...
async def gspider(rk): 
   ...
   if user:      
        ...
        try:
          from ub.plugs.sqls.globelmute_sql import globelmute          
        except:
   	     pass        
   else:
...
async def gspider(rk):    
   ...
   try:
     from ub.plugs.sqls.globelmute_sql import unglobelmute
   except:
   	return
   if rk.is_private:       
   	user = rk.chat ; reason = rk.pattern_match.group(1) ; chat_title = 'PM'  
...
async def gspider(rk): 
   ...
   if user:      
        ...
        try:
          from ub.plugs.sqls.gmute_sql import gmute            
        except:
   	     pass
        try:
...
async def gspider(rk):
   ...
   if user:      
        ...
        try:
          from ub.plugs.sqls.gmute_sql import ungmute
        except:
   	     pass
        try:
