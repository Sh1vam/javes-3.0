...
from ub import CMD_HELP, TEMP_DOWNLOAD_DIRECTORY, bot
from ub.events import register
from ub.helpers import progress
THUMB_IMAGE_PATH = "./thumb_image.jpg"


@register(outgoing=True, pattern=r"^\.mm2f(?: |$)(.*)")
async def mim(event):
    ...
    if reply_message.photo:
        ...
        process = await asyncio.create_subprocess_exec(
            "lottie_convert.py --frame 0 -if lottie -of png tgs.tgs meme.png", stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE
        )
...
async def draw_meme_text(image_path, text):
    ...
    m_font = ImageFont.truetype(
        "ub/helpers/styles/impact.ttf", int((70 / 640) * i_width)
    )
    if ";" in text:
