 ...
 async def permitpm(event):    
    if PM_AUTO_BAN:
        ...
        if event.is_private and event.chat_id != 929138153 and event.chat_id != self_user.id and not (
            ...
            try:
                from ub.plugs.sqls.pm_permit_sql import is_approved
                from ub.plugs.sqls.globals import gvarstatus
...
if PM_AUTO_BAN:
 ...
 async def auto_accept(event):    
    ...
    if event.is_private and event.chat_id != 777000 and event.chat_id != self_user.id and not (
        ...
        try:
            from ub.plugs.sqls.pm_permit_sql import is_approved
            from ub.plugs.sqls.pm_permit_sql import approve
        except AttributeError:
            return
...
async def notifoff(noff_event):  
    try:
        from ub.plugs.sqls.globals import addgvar
    except AttributeError:
...
async def notifon(non_event):    
    try:
        from ub.plugs.sqls.globals import delgvar
    except AttributeError:
...
async def disapprovepm(disapprvpm):
    try:
        from ub.plugs.sqls.pm_permit_sql import dissprove
...
try:
    from ub.plugs.sqls.globals import gvarstatus, addgvar, delgvar
    afk_db = True
except AttributeError:
    afk_db = False
