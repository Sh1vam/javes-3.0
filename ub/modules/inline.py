...
from ub.modules.cobra import *
from ub.modules import cobra
import io
import json
import math
import os
...
from ub.utils import remove_plugin,load_module
...
from ub import ALIVE_NAME, CMD_HELP, CMD_LIST
...
if tebot:
 ...
 async def on_plug_in_callback_query_handler(event):
        ...
        if os.path.exists("./ub/secrets.txt"):
            jsondata = json.load(open("./ub/secrets.txt"))
            try:
...
if tebot:
 ...
 async def inline_handler(event):
  ...
  if event.query.user_id == me.id and match:
            ...
            builder = event.builder
            secret = os.path.join("./ub", "secrets.txt")
            try:
