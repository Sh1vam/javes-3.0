...
async def inline_id_handler(event: events.InlineQuery.Event):
    ...
    if event.query.user_id == me.id:
        ...
        try:
            sh1vam = builder.document(f"./ub/modules/{input_str}.py",title=f"{input_str}.py",description=f"Module {input_str} Found",text=f"{input_str}.py use .neko to paste in neko bin",buttons=[[Button.switch_inline("Search Again", query="send ", same_peer=True)],], )
...
async def inline_id_handler(event: events.InlineQuery.Event):
    ...
    if event.query.user_id == me.id:
        input_str = event.pattern_match.group(1)
        try:
            sh1vam = builder.document(f"./ub/modules/sql_helper/{input_str}.py",title=f"{input_str}.py",description=f"Module {input_str} Found",text=f"{input_str}.py use .neko to paste in neko bin",buttons=[[Button.switch_inline("Search Again", query="send ", same_peer=True)],], )
            await event.answer([sh1vam])
            return
        except:
