...
async def inline_id_handler(event: events.InlineQuery.Event):
    ...
    if event.query.user_id == me.id:
            s = builder.article(title="Inline Help",description="For Inline commands",text="for cmd like yt and pic u need to specify Querry;number and for base64 :- base64 (en|de) querry;for whisper u need @username Querry and for button see .chkbutton",buttons=[[Button.switch_inline("Inline Help", query="ihelp", same_peer=True)],[Button.switch_inline("Base64", query="base64 ", same_peer=True),Button.switch_inline("Help", query="helpme", same_peer=True),Button.switch_inline("Torrent", query="tor ", same_peer=True),Button.switch_inline("You Tube", query="yt ", same_peer=True)],[Button.switch_inline("Pic", query="pic ", same_peer=True),Button.switch_inline("Exec", query="exec ", same_peer=True),Button.switch_inline("Eval", query="eval ", same_peer=True),Button.switch_inline("Bot Logs", query="logs", same_peer=True)],[Button.switch_inline("Hash", query="hash ", same_peer=True),Button.switch_inline("Whisper", query="secret ", same_peer=True),Button.switch_inline("Url Button", query="secrete ", same_peer=True),Button.switch_inline("Send Module", query="send ", same_peer=True)],[Button.inline('❌ Close menu', b'close')],] )
