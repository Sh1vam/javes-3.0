    ...
    if event.reply_to_msg_id:
        try:
            downloaded_file_name = await event.client.download_media(  # pylint:disable=E0602
                ...
                "/root/ub/ub/modules/"  # pylint:disable=E0602
            )
...
async def send(event):
    ...
    try:
        the_plugin_file = "./ub/modules/{}.py".format(input_str)
        start = datetime.now()
...
async def install(event):
    ...
    for ixo in total_doxx:
        await event.edit(text) ; mxo = cui[ixo].id ; downloaded_file_name = await event.client.download_media(await client.get_messages(chat, ids=mxo), "ub/modules/")
        if "(" not in downloaded_file_name:
