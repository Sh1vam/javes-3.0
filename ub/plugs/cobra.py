...
def paginate_help(page_number, loaded_plugins, prefix):
    ...
    plugs = [custom.Button.inline(
        "{} {} {}".format(random.choice(list(multi)), x,random.choice(list(mult2i))),
        data="bus_plugin_{}".format(x))
        for x in helpable_plugins]
    pairs =  list(zip(plugs[0::number_of_cols],plugs[1::number_of_cols],plugs[2::number_of_cols]))
    if len(plugs) % number_of_cols == 1:
        pairs.append((plugs[-1],))
