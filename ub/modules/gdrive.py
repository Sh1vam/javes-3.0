...
from ub import (G_DRIVE_CLIENT_ID, G_DRIVE_CLIENT_SECRET,
...
async def show_current_gdrove_folder(event):
    if parent_id:
        ...
        await event.edit(
            f"My ub is currently uploading files [here]({folder_link})")
    else:
        await event.edit(
            f"My ub is currently uploading files to the root of my Google Drive storage.\
